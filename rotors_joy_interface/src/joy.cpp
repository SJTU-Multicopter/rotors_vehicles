...
Joy::Joy() {
  ...
  pnh.param("axis_roll_", axes_.roll, 3); //chg
  pnh.param("axis_pitch_", axes_.pitch, 4); //chg
  pnh.param("axis_thrust_", axes_.thrust, 1); //chg
...
void Joy::JoyCallback(const sensor_msgs::JoyConstPtr& msg) {
  ...
  Publish();
  ROS_INFO_STREAM("control_msg_.thrust.z =" << control_msg_.thrust.z);
}
...
int main(int argc, char** argv) {
  ...
  ROS_WARN_STREAM("Joy initialized");
